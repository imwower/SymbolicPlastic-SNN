...
class SnnRunner:
    def __init__(self, cfg: RunnerConfig | None = None, seed: int = 1234) -> None:
        ...
        self.alias_tbl = AliasForTile(prob=prob, alias=alias)
    ...
    def step(self, x_t: np.ndarray) -> Optional[Dict[str, Any]]:
        ...
        if self._step_index > 0 and (self._step_index % period == 0):
            ...
            prob, alias = build_alias(new_prob_q)
            self.alias_tbl = AliasForTile(prob=prob, alias=alias)
            reweighted = True
    ...
    def _aggregate_events(self, events: List[BlockEvent]) -> np.ndarray:
        I = np.zeros(self.N, dtype=np.int32)
        if not events:
            return I
        for ev in events:
            if ev.capped:
                # If capped, distribute evenly across the tile as a fallback
                start = ev.post_tile * self.tile_size
                end = start + self.tile_size
                if ev.total_k > 0:
                    ...
                    if add > 0:
                        I[start:end] += add
                continue
            base = ev.post_tile * self.tile_size
            I[base + ev.indices] += ev.k.astype(np.int32)
        return I
